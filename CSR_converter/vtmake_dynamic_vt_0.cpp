...
int main(int argc, char** argv) {
	...
	if (openFile.is_open()) {
		...
		for (uint64_t i = 0; i < v; i++) {
			...
			if(i <= v/2) {
				...
				for (uint64_t j = 0; j < v_cnt; j++) { // 그 대상들에 대해서
					...
					uint64_t index = edge / ceil((float)(v_num/2)); // 아래로 가져다 붙이기 때문에 몇번째에 해당하는지...
					if(index == 2) {
						index--;
					}
					// 1번 블럭의 경우
					if(edge >=2) {
						new_csr[i+(1.5*v)].push_back(stoull(tmp));
					} else {
						new_csr[i+0.5*v*index].push_back(stoull(tmp));
					}
				}
			} else {
				uint64_t v_cnt = row[i+1] - row[i]; // 몇개 vertex인지....(해당 row에)
				for (uint64_t j = 0; j < v_cnt; j++) { // 그 대상들에 대해서
					...
					new_csr[0.5*v + i + v*index].push_back(stoull(tmp));
				}
			}
		}
		openFile.close();
		...
		*/
		...
		string output_path = p + "_dynamic_vt_0.txt";
